...
// Asset maps to science_assets. Text column is title (not word_text).
type Asset struct {
...
func (Asset) TableName() string { return "science_assets" }
...
// ItemDTO is the list/patch response shape. JSON uses title (not wordText).
// Group/list child arrays use "items" (plan/spec); route is /science/items.
type ItemDTO struct {
...
// ObjectPutter mirrors english store deps; Science*Key used by Task 6 generate.
type ObjectPutter interface {
	...
	ScienceGlyphKey(kpID int64) string
	ScienceSenseKey(kpID int64) string
	ScienceSpeechKey(kpID int64) string
}
...
func speechPublicURL(kpID int64) string {
	return fmt.Sprintf("%s/api/v1/science/items/%d/speech.mp3", publicBase(), kpID)
}
...
func (s *Service) synthesizeSpeech(ctx context.Context, asset Asset) ([]byte, error) {
	...
	text := strings.TrimSpace(asset.Title)
	if text == "" {
...
func (s *Service) storeSpeech(ctx context.Context, asset *Asset, mp3 []byte) error {
	...
	if _, err := s.store.PutBytes(ctx, storage.ScienceSpeechObjectKey(asset.KpID), mp3, "audio/mpeg"); err != nil {
		return err
	}
...
func (s *Service) SpeechMP3(ctx context.Context, kpID int64) ([]byte, error) {
	...
	if s.store != nil && strings.TrimSpace(asset.SpeechAudioURL) != "" {
		if data, err := s.store.GetBytes(ctx, s.store.ScienceSpeechKey(kpID)); err == nil && len(data) > 0 {
			return data, nil
		}
	}
...
func (s *Service) PatchOverride(kpID int64, override *bool) (ItemDTO, error) {
	var asset Asset
	if err := s.db.First(&asset, "kp_id = ?", kpID).Error; err != nil {
		return ItemDTO{}, err
	}
...
func (s *Service) GenerateGlyph(ctx context.Context, kpID int64) (ItemDTO, error) {
	...
	png, err := s.renderer.RenderEnglishPNG(asset.Title)
	if err != nil {
		return ItemDTO{}, err
	}
	key, err := s.store.PutPNG(ctx, storage.ScienceGlyphObjectKey(kpID), png)
	if err != nil {
		return ItemDTO{}, err
	}
	...
	asset.GlyphImageURL = fmt.Sprintf("%s/api/v1/science/items/%d/glyph.png", publicBase, kpID)
	_ = key
	asset.UpdatedAt = time.Now().UTC()
	if err := s.db.Save(&asset).Error; err != nil {
		return ItemDTO{}, err
...
func (s *Service) GlyphPNG(ctx context.Context, kpID int64) ([]byte, error) {
	...
	return s.store.GetPNG(ctx, s.store.ScienceGlyphKey(kpID))
}
...
func (s *Service) GenerateSense(ctx context.Context, kpID int64) (ItemDTO, error) {
	if s.store == nil || s.images == nil || s.cfg == nil {
		return ItemDTO{}, fmt.Errorf("义图生成未就绪（检查 MinIO 与图片模型配置）")
	}
	var asset Asset
	if err := s.db.First(&asset, "kp_id = ?", kpID).Error; err != nil {
		return ItemDTO{}, err
	...
	if !asset.EffectiveNeedsSenseImage() {
		return ItemDTO{}, fmt.Errorf("该词按规则不需要义图")
	}
	ai, err := s.cfg.LoadImageModels(ctx)
	if err != nil {
		return ItemDTO{}, fmt.Errorf("加载图片模型失败: %w", err)
	}
	providers := listImageProviders(ai)
	if len(providers) == 0 {
		return ItemDTO{}, fmt.Errorf("配置中心没有可用的图片模型")
	}
	prompt := sense.PromptScience(asset.Title)
	var png []byte
	var lastErr error
	...
	if lastErr != nil {
		return ItemDTO{}, lastErr
	}
	if _, err := s.store.PutPNG(ctx, storage.ScienceSenseObjectKey(kpID), png); err != nil {
		return ItemDTO{}, err
	}
	publicBase := strings.TrimRight(os.Getenv("APP_PUBLIC_BASE"), "/")
	if publicBase == "" {
	...
	asset.SenseImageURL = fmt.Sprintf("%s/api/v1/science/items/%d/sense.png", publicBase, kpID)
	asset.UpdatedAt = time.Now().UTC()
	if err := s.db.Save(&asset).Error; err != nil {
		return ItemDTO{}, err
	}
...
func (s *Service) SensePNG(ctx context.Context, kpID int64) ([]byte, error) {
	...
	return s.store.GetPNG(ctx, s.store.ScienceSenseKey(kpID))
}
...
func (s *Service) BatchGenerateSenses(ctx context.Context, moduleCode string, workers, maxRetries int) (GlyphBatchResult, error) {
	...
	for _, asset := range assets {
		...
		pending = append(pending, job{kpID: asset.KpID, char: asset.Title})
	}
