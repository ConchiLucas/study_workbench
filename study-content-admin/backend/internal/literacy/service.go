...
type Asset struct {
	...
	CharText                string    `gorm:"column:char_text" json:"charText"`
...
func (Asset) TableName() string { return "literacy_assets" }
...
type ListResult struct {
	...
	Chars  []CharDTO  `json:"chars,omitempty"`
}
...
type ObjectPutter interface {
	...
	GlyphKey(kpID int64) string
	SenseKey(kpID int64) string
	SpeechKey(kpID int64) string
}

type GlyphRenderer interface {
	RenderPNG(char string) ([]byte, error)
}
...
func pickSpeechProvider(ai configclient.AIConfiguration, asset Asset) (configclient.AIProvider, error) {
	// 第2组先用小米 MiMo TTS；其余组走默认（Grok）
	if asset.ModuleCode == "g2" || strings.Contains(asset.ModuleName, "第2组") {
		return tts.PickProviderByID(ai, "xiaomi-mimo-tts")
	}
	return tts.PickProvider(ai)
}

func speechPublicURL(kpID int64) string {
	return fmt.Sprintf("%s/api/v1/literacy/chars/%d/speech.mp3", publicBase(), kpID)
}
...
func (s *Service) synthesizeSpeech(ctx context.Context, asset Asset) ([]byte, error) {
	...
	text := strings.TrimSpace(asset.CharText)
	if text == "" {
		return nil, fmt.Errorf("字文本为空")
	}
...
func (s *Service) storeSpeech(ctx context.Context, asset *Asset, mp3 []byte) error {
	...
	if _, err := s.store.PutBytes(ctx, storage.SpeechObjectKey(asset.KpID), mp3, "audio/mpeg"); err != nil {
		return err
	}
...
func (s *Service) SpeechMP3(ctx context.Context, kpID int64) ([]byte, error) {
	...
	if s.store != nil && strings.TrimSpace(asset.SpeechAudioURL) != "" {
		if data, err := s.store.GetBytes(ctx, s.store.SpeechKey(kpID)); err == nil && len(data) > 0 {
			return data, nil
		}
	}
...
func (s *Service) RegenerateSpeech(ctx context.Context, kpID int64) (CharDTO, error) {
	var asset Asset
	if err := s.db.First(&asset, "kp_id = ?", kpID).Error; err != nil {
		return CharDTO{}, err
	}
	...
	for i, asset := range assets {
		...
		if err != nil {
			...
			if len(out.Errors) < 10 {
				out.Errors = append(out.Errors, fmt.Sprintf("%s(%d): %v", asset.CharText, asset.KpID, err))
			}
			continue
		}
		if err := s.storeSpeech(ctx, &asset, mp3); err != nil {
...
func (s *Service) List(view string, needsFilter *bool) (ListResult, error) {
	...
	dtos := make([]CharDTO, 0, len(assets))
...
func (s *Service) PatchOverride(kpID int64, override *bool) (CharDTO, error) {
	var asset Asset
	if err := s.db.First(&asset, "kp_id = ?", kpID).Error; err != nil {
		return CharDTO{}, err
	}
...
func (s *Service) GenerateGlyph(ctx context.Context, kpID int64) (CharDTO, error) {
	if s.store == nil || s.renderer == nil {
		return CharDTO{}, fmt.Errorf("字图生成未就绪（检查 MinIO 与字体）")
	}
	var asset Asset
	if err := s.db.First(&asset, "kp_id = ?", kpID).Error; err != nil {
		return CharDTO{}, err
	}
	png, err := s.renderer.RenderPNG(asset.CharText)
	if err != nil {
		return CharDTO{}, err
	}
	key, err := s.store.PutPNG(ctx, storage.GlyphObjectKey(kpID), png)
	if err != nil {
		return CharDTO{}, err
	}
	...
	asset.GlyphImageURL = fmt.Sprintf("%s/api/v1/literacy/chars/%d/glyph.png", publicBase, kpID)
	_ = key
	asset.UpdatedAt = time.Now().UTC()
	if err := s.db.Save(&asset).Error; err != nil {
		return CharDTO{}, err
...
func (s *Service) GlyphPNG(ctx context.Context, kpID int64) ([]byte, error) {
	...
	return s.store.GetPNG(ctx, s.store.GlyphKey(kpID))
}

func (s *Service) BatchGenerateGlyphs(ctx context.Context) (GlyphBatchResult, error) {
	if s.store == nil || s.renderer == nil {
		return GlyphBatchResult{}, fmt.Errorf("字图生成未就绪（检查 MinIO 与字体）")
	}
	var assets []Asset
	if err := s.db.Where("glyph_image_url = '' OR glyph_image_url IS NULL").
		Order("module_order ASC, kp_order ASC, kp_id ASC").
		Find(&assets).Error; err != nil {
		return GlyphBatchResult{}, err
	}
...
func (s *Service) GenerateSense(ctx context.Context, kpID int64) (CharDTO, error) {
	if s.store == nil || s.images == nil || s.cfg == nil {
		return CharDTO{}, fmt.Errorf("义图生成未就绪（检查 MinIO 与图片模型配置）")
	}
	var asset Asset
	if err := s.db.First(&asset, "kp_id = ?", kpID).Error; err != nil {
		return CharDTO{}, err
	...
	if !asset.EffectiveNeedsSenseImage() {
		return CharDTO{}, fmt.Errorf("该字按规则不需要义图")
	}
	ai, err := s.cfg.LoadImageModels(ctx)
	if err != nil {
		return CharDTO{}, fmt.Errorf("加载图片模型失败: %w", err)
	}
	providers := listImageProviders(ai)
	if len(providers) == 0 {
		return CharDTO{}, fmt.Errorf("配置中心没有可用的图片模型")
	}
	prompt := sense.Prompt(asset.CharText)
	var png []byte
	var lastErr error
	...
	if lastErr != nil {
		return CharDTO{}, lastErr
	}
	if _, err := s.store.PutPNG(ctx, storage.SenseObjectKey(kpID), png); err != nil {
		return CharDTO{}, err
	}
	publicBase := strings.TrimRight(os.Getenv("APP_PUBLIC_BASE"), "/")
	if publicBase == "" {
	...
	asset.SenseImageURL = fmt.Sprintf("%s/api/v1/literacy/chars/%d/sense.png", publicBase, kpID)
	asset.UpdatedAt = time.Now().UTC()
	if err := s.db.Save(&asset).Error; err != nil {
		return CharDTO{}, err
	}
...
func (s *Service) SensePNG(ctx context.Context, kpID int64) ([]byte, error) {
	...
	return s.store.GetPNG(ctx, s.store.SenseKey(kpID))
}
...
func (s *Service) BatchGenerateSenses(ctx context.Context, moduleCode string, workers, maxRetries int) (GlyphBatchResult, error) {
	...
	for _, asset := range assets {
		...
		pending = append(pending, job{kpID: asset.KpID, char: asset.CharText})
	}
